...
## Estimating thermodynamic properties from nucleic acid sequence: the nearest-neighbor method
...
In general the predictions from the nearest neighbor method agree reasonably well with experimental results, but some unexpected outlying sequences, calling for further insights, do exist. Finally, we should also mention the increased accuracy provided by single molecule unzipping assays which provide a wealth of new insight into the thermodynamics of DNA hybridization and the validity of the nearest-neighbour model as well.

### Expanded neighbor models
A more realistic way of modeling the behavior of nucleic acids would seem to be to have parameters that depend on the neighboring groups on both sides of a nucleotide, giving a table with entries like "TCG/AGC". However, this would involve around 32 groups for Watson-Crick pairing and even more for sequences containing mismatches; the number of DNA melting experiments needed to get reliable data for so many groups would be inconveniently high. However, other means exist to access thermodynamic parameters of nucleic acids:  microarray technology allows hybridization monitoring of tens of thousands sequences in parallel. This data, in combination with molecular adsorption theory allows the determination of many thermodynamic parameters in a single experiment and to go beyond the nearest neighbor model.

Expanded models are very commonly used in RNA secondary structure prediction. In this field, a "nearest neighbor" model can include string of not only two, but also several bases. As with the earlier iteration, the free energy is provided 37 °C and enthalpy values may be provided for calculating free energies for other temperatures. These models can include the thermodynamic parameters for fragments such as:

Helix regions, generally on the scale of two bases, e.g. AU:UA (5'-3':3'-5' notation like before).

GU pairs, which are scored similarly to helix regions, but longer fragments may be used, e.g. GGUC:CUGG.

Dangling (single-strand) ends, which can have a stacking energy contribution. Generally described as fragments of two.

Terminal mismatches, where a helix ends at a mixmatch. e.g. CA:GA.

Hairpin, bulge, internal, multibranch, and exterior loops, where the length, the nature of the terminal mismatch (for hairpin only), the base composition of the loop (again, due to stacking) can all have effects on its stability.

Coaxial stacking is technically under the umbrella of tertiary structure, but secondary structure predictors can recognize these features and score accordingly.
