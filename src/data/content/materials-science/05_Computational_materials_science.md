...
Cluster dynamics is a rate theory model that groups concentrations of particles into clusters that can be solved with a set of ordinary differential equations. By clustering particles (typically gas bubbles) of similar size, these few sets of equations are able to describe the entire rate evolution of the system. Cluster dynamics allows for simulations over large length and time scales at the cost of being able to track every atom in the material as is capable in molecular dynamics.
