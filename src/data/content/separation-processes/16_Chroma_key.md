...
There is also a form of colour keying that uses light spectrum invisible to human eye. Called Thermo-Key, it uses infrared as the key colour, which would not be replaced by background image during postprocessing.
...
There is some use of the specific full-intensity magenta colour #FF00FF in digital colour images to encode (1-bit) transparency; this is sometimes referred to as "magic pink". This is not a photographic technique, and the extraction of the foreground from the background is trivial.
