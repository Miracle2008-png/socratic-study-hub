import { Calculator, FlaskConical, Atom, Dna, Zap, Wrench } from 'lucide-react';
...
export const formulaTree: CategoryNode[] = [
  ...
  {
    id: 'thermodynamics', label: 'Thermodynamics', color: '#ef4444',
    formulas: [
      { id:'th1', name:'First Law (Closed)',       latex:'$\\Delta U=Q-W$',                                                                description:'Energy conservation for a closed system.',                            tags:['thermo','energy'] },
      { id:'th2', name:'Enthalpy Definition',      latex:'$H=U+PV$',                                                                       description:'Useful for constant pressure processes.',                             tags:['thermo','enthalpy'] },
      { id:'th3', name:'Ideal Gas Law',            latex:'$PV=nRT$',                                                                       description:'Equation of state for an ideal gas.',                                 tags:['thermo','gas'] },
      { id:'th4', name:'Carnot Efficiency',        latex:'$\\eta=1-\\dfrac{T_C}{T_H}$',                                                    description:'Maximum theoretical heat engine efficiency.',                         tags:['thermo','carnot'] },
      { id:'th5', name:'Entropy Change (Rev)',     latex:'$\\Delta S=\\int\\dfrac{dQ_{rev}}{T}$',                                          description:'Definition of entropy change.',                                       tags:['thermo','entropy'] },
      { id:'th6', name:'Gibbs Free Energy',        latex:'$G=H-TS$',                                                                       description:'Predicts spontaneity at constant T and P.',                           tags:['thermo','gibbs'] },
      { id:'th7', name:'Maxwell Relations',        latex:'$\\left(\\dfrac{\\partial T}{\\partial V}\\right)_S=-\\left(\\dfrac{\\partial P}{\\partial S}\\right)_V$', description:'Example of a Maxwell relation.',                  tags:['thermo','maxwell'] },
      { id:'th8', name:'Clapeyron Equation',       latex:'$\\dfrac{dP}{dT}=\\dfrac{L}{T\\Delta V}$',                                       description:'Slope of phase boundary in P-T diagram.',                             tags:['thermo','phase'] },
    ]
  },
  {
    id: 'chem_eng_transfer', label: 'Transfer Processes (1, 2, 3)', color: '#f59e0b',
      ...
      {
        id: 'electrical_eng', label: 'Electrical Engineering', color: '#eab308',
        children: [
          {
            id: 'ee_circuits', label: 'Circuit Theory', color: '#eab308',
            formulas: [
              { id:'ee_c1', name:"Ohm's Law (Phasor)",       latex:'$\\mathbf{V} = \\mathbf{I} \\mathbf{Z}$',                                         description:'Voltage, current, and impedance in AC circuits.', tags:['circuits','AC'] },
              { id:'ee_c2', name:"Kirchhoff's Voltage Law",  latex:'$\\sum_{k=1}^n V_k = 0$',                                                         description:'Sum of voltages around any closed loop is zero.', tags:['circuits','KVL'] },
              { id:'ee_c3', name:"Kirchhoff's Current Law",  latex:'$\\sum_{k=1}^n I_k = 0$',                                                         description:'Sum of currents entering a node is zero.', tags:['circuits','KCL'] },
              { id:'ee_c4', name:'Real Power',               latex:'$P = V_{\\text{rms}} I_{\\text{rms}} \\cos\\theta$',                              description:'Average power dissipated by a load.', tags:['circuits','power'] },
              { id:'ee_c5', name:'Reactive Power',           latex:'$Q = V_{\\text{rms}} I_{\\text{rms}} \\sin\\theta$',                              description:'Power oscillating between source and reactive load.', tags:['circuits','power'] },
              { id:'ee_c6', name:'Apparent Power',           latex:'$\\mathbf{S} = \\mathbf{V} \\mathbf{I}^* = P + jQ$',                              description:'Complex power in AC circuits.', tags:['circuits','power'] },
              { id:'ee_c7', name:'Thevenin Voltage',         latex:'$V_{\\text{th}} = V_{\\text{oc}}$',                                               description:'Open-circuit voltage at terminals.', tags:['circuits','thevenin'] },
              { id:'ee_c8', name:'Maximum Power Transfer',   latex:'$Z_L = Z_{\\text{th}}^*$',                                                        description:'Load impedance for maximum power transfer.', tags:['circuits','power'] },
            ]
          },
          {
            id: 'ee_signals', label: 'Signals & Systems', color: '#8b5cf6',
            formulas: [
              { id:'ee_s1', name:'Continuous Convolution',   latex:'$y(t) = (x * h)(t) = \\int_{-\\infty}^{\\infty} x(\\tau) h(t - \\tau) d\\tau$',   description:'Output of an LTI system given impulse response h(t).', tags:['signals','convolution'] },
              { id:'ee_s2', name:'Discrete Convolution',     latex:'$y[n] = (x * h)[n] = \\sum_{k=-\\infty}^{\\infty} x[k] h[n - k]$',                description:'Discrete-time LTI system output.', tags:['signals','convolution'] },
              { id:'ee_s3', name:'Z-Transform',              latex:'$X(z) = \\sum_{n=-\\infty}^{\\infty} x[n] z^{-n}$',                               description:'Converts discrete time-domain signal to complex frequency domain.', tags:['signals','transforms'] },
              { id:'ee_s4', name:'Nyquist Rate',             latex:'$f_s \\geq 2 f_{\\text{max}}$',                                                   description:'Minimum sampling rate to avoid aliasing.', tags:['signals','sampling'] },
              { id:'ee_s5', name:'Discrete Fourier Transform',latex:'$X[k] = \\sum_{n=0}^{N-1} x[n] e^{-j 2\\pi k n / N}$',                           description:'DFT of a sequence of N samples.', tags:['signals','fourier'] },
            ]
          },
          {
            id: 'ee_control', label: 'Control Systems', color: '#10b981',
            formulas: [
              { id:'ee_cs1', name:'Transfer Function',       latex:'$H(s) = \\dfrac{Y(s)}{X(s)}$',                                                    description:'Ratio of output Laplace transform to input Laplace transform.', tags:['control','transfer function'] },
              { id:'ee_cs2', name:'Closed-Loop Transfer Func',latex:'$T(s) = \\dfrac{G(s)}{1 + G(s)H(s)}$',                                           description:'Overall transfer function with feedback loop.', tags:['control','feedback'] },
              { id:'ee_cs3', name:'PID Controller',          latex:'$u(t) = K_p e(t) + K_i \\int_0^t e(\\tau)d\\tau + K_d \\dfrac{de(t)}{dt}$',      description:'Proportional-Integral-Derivative control law.', tags:['control','PID'] },
              { id:'ee_cs4', name:'Damping Ratio',           latex:'$\\zeta = \\dfrac{c}{2\\sqrt{km}}$',                                              description:'Determines the transient response of a 2nd order system.', tags:['control','damping'] },
              { id:'ee_cs5', name:'Steady-State Error',      latex:'$e_{ss} = \\lim_{s \\to 0} s E(s)$',                                              description:'Final value theorem applied to the error signal.', tags:['control','error'] },
            ]
          }
        ]
      },
      {
        id: 'mech_civil_eng', label: 'Mechanical & Civil Engineering', color: '#f43f5e',
        children: [
          {
            id: 'solid_mechanics', label: 'Solid Mechanics & Statics', color: '#f43f5e',
            formulas: [
          ...
          {
            id: 'machine_design', label: 'Machine Design & Failure Theory', color: '#0ea5e9',
            formulas: [
              { id:'md_1', name:'Von Mises Stress',          latex:'$\\sigma_v = \\sqrt{\\sigma_1^2 - \\sigma_1\\sigma_2 + \\sigma_2^2}$',             description:'Equivalent stress for distortion energy theory.', tags:['design','failure'] },
              { id:'md_2', name:'Factor of Safety (Yield)',  latex:'$n = \\dfrac{S_y}{\\sigma_{\\text{max}}}$',                                       description:'Ratio of yield strength to maximum operating stress.', tags:['design','safety'] },
              { id:'md_3', name:'Goodman Line (Fatigue)',    latex:'$\\dfrac{\\sigma_a}{S_e} + \\dfrac{\\sigma_m}{S_{ut}} = \\dfrac{1}{n}$',          description:'Fatigue failure criteria for alternating and mean stresses.', tags:['design','fatigue'] },
              { id:'md_4', name:"Miner's Rule",              latex:'$\\sum \\dfrac{n_i}{N_i} = C$',                                                   description:'Cumulative fatigue damage.', tags:['design','fatigue'] },
              { id:'md_5', name:'Euler Buckling Load',       latex:'$P_{\\text{cr}} = \\dfrac{\\pi^2 E I}{(K L)^2}$',                                 description:'Critical load for column buckling.', tags:['design','buckling'] },
            ]
          }
        ]
      }
