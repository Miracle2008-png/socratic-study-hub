...
import { Mail, Lock, BrainCircuit, Sparkles, CheckCircle, Loader2, Eye, EyeOff, ArrowRight, ArrowLeft, KeyRound } from 'lucide-react';
...
const SignInProgress: React.FC<{ email: string; onComplete: () => void }> = ({ email, onComplete }) => {
  ...
  useEffect(() => {
    let stepIndex = 0;
    const runStep = () => {
      if (stepIndex >= LOGIN_STEPS.length) { setTimeout(onComplete, 300); return; }
      setCurrentStep(stepIndex);
      const dur = LOGIN_STEPS[stepIndex].duration;
      setTimeout(() => { setCompletedSteps(prev => [...prev, stepIndex]); stepIndex++; runStep(); }, dur);
    };
    runStep();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 32px', textAlign: 'center', animation: 'fadeIn 0.4s ease' }}>
      <div style={{ position: 'relative', width: 96, height: 96, marginBottom: 32 }}>
        ...
        <div style={{ position: 'absolute', inset: 8, borderRadius: '50%', background: 'linear-gradient(135deg, var(--color-base-alt), var(--color-base-deep))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 800, color: 'var(--color-accent)', textTransform: 'uppercase' }}>
          {name.charAt(0)}
        </div>
      </div>
      <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-text-primary)', marginBottom: 6 }}>Hey, {name}!</h2>
      <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 40 }}>Setting up your personalised workspace…</p>
      <div style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 16 }}>
        {LOGIN_STEPS.map((step, i) => {
          ...
          return (
            <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: 14, opacity: isPending ? 0.3 : 1, transition: 'opacity 0.4s ease' }}>
...
export const LoginScreen: React.FC = () => {
  ...
  const triggerLogin = (emailAddr: string) => { setPendingEmail(emailAddr); setShowProgress(true); };
  const handleProgressComplete = () => { mockLogin(pendingEmail); };
  ...
  const handleGoogleLogin = () => {
    setLoading(true);
    setTimeout(() => { setLoading(false); triggerLogin('scholar@gmail.com'); }, 400);
  };
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (err) {
      // If Supabase not configured, fall back to mock login
      if (err.message?.includes('fetch') || err.message?.includes('Failed') || err.message?.includes('placeholder')) {
        triggerLogin(email);
      } else {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (err) {
      ...
      setView('check_email');
      setSuccess(`We've sent a confirmation link to ${email}. Check your inbox!`);
    }
  };
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (err) {
      if (err.message?.includes('fetch') || err.message?.includes('Failed') || err.message?.includes('placeholder')) {
        setView('check_email');
        setSuccess(`If an account exists for ${email}, you'll receive a reset link shortly.`);
      } else {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', width: '100%', maxWidth: 420 }}>
        <div className="luxury-card" style={{ width: '100%', padding: showProgress ? 0 : '44px 40px', position: 'relative', zIndex: 100, overflow: 'hidden', transition: 'padding 0.4s' }}>
          {showProgress ? (
            <SignInProgress email={pendingEmail} onComplete={handleProgressComplete} />
          ) : view === 'check_email' ? (
            // ── Check Email Screen ──────────────────────────────────────────
            <div style={{ textAlign: 'center', animation: 'floatUp 0.5s ease both' }}>
              ...
              <p style={{ color: 'var(--color-text-muted)', fontSize: 14, lineHeight: 1.6, marginBottom: 28 }}>
                {success || `We've sent an email to ${email}. Click the link inside to continue.`}
              </p>
            ...
            <>
              ...
              <div style={{ textAlign: 'center', marginBottom: 28, animation: 'floatUp 0.5s ease both' }}>
                ...
                <p style={{ color: 'var(--color-text-muted)', fontSize: 13 }}>Enter your email and we'll send you a secure reset link.</p>
              </div>

              {error && (
                <div style={{ background: 'rgba(239,68,68,0.08)', color: '#ef4444', padding: '12px 16px', borderRadius: 10, marginBottom: 16, fontSize: 13, border: '1px solid rgba(239,68,68,0.2)', animation: 'floatUp 0.3s ease' }}>
                  {error}
                </div>
              )}
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 28, animation: 'floatUp 0.5s ease both' }}>
                ...
                <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 6, letterSpacing: '-0.5px', color: 'var(--color-text-primary)' }}>Create Account</h1>
              ...
              <button type="button" className="bypass-btn" onClick={() => triggerLogin('miraclechimdindu2008@gmail.com')}>
                ⚡ Admin Quick Login
              </button>
            </>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 32, animation: 'floatUp 0.5s ease both' }}>
                ...
                <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 8, letterSpacing: '-0.5px', color: 'var(--color-text-primary)' }}>Socratic Study Hub</h1>
                <p style={{ color: 'var(--color-text-muted)', fontSize: 14, lineHeight: 1.5 }}>Sign in to continue your learning journey.</p>
              </div>

              {error && (
                <div style={{ background: 'rgba(239,68,68,0.08)', color: '#ef4444', padding: '12px 16px', borderRadius: 10, marginBottom: 20, fontSize: 13, border: '1px solid rgba(239,68,68,0.2)', animation: 'floatUp 0.3s ease' }}>
                  {error}
                </div>
              )}
              ...
              <form onSubmit={handleSignIn} style={{ animation: 'floatUp 0.5s 0.15s ease both' }}>
                ...
                <button type="submit" className="login-submit" disabled={loading}>
                  {loading ? <><Loader2 size={16} style={{ animation: 'spin 0.9s linear infinite' }} /> Processing…</> : <>Sign In <ArrowRight size={16} /></>}
                </button>
              </form>
